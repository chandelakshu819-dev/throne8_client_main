...
const PaymentStep: React.FC<PaymentStepProps> = ({
    ...
    const handleBookSession = async () => {
        ...
        try {
            // scheduledAt banana â€” date + slotTime se
            ...
            await SessionService.bookSession({
                sessionId: String(selectedService?.id || ""),  // âœ… ye ab sessionId hai
                mentorId,
                availabilityId,
                slotTime,
                scheduledAt,
            ...
            // Success â€” mentor profile par redirect
            onBookingSuccess();
        } catch (error: any) {
            console.error("Booking failed:", error.message);
            console.error("Full error:", error?.response?.data); // ye undefined rahega
            console.error("Error object keys:", Object.keys(error)); // ðŸ‘ˆ add ye
            setBooking(false);
        }
    };

    // ðŸ‘‡ Booking loader screen
    if (booking) {
        return (
        ...
        <div style={{ minHeight: "100vh", background: C.bg, padding: "32px 16px" }}>
            ...
            <div style={{ maxWidth: "700px", margin: "0 auto", borderRadius: "24px", padding: "40px", background: C.surface, border: `1px solid ${C.border}`, boxShadow: "0 20px 60px rgba(74,55,40,0.15)" }}>
                ...
                <div style={{ borderRadius: "16px", padding: "20px", background: C.bg, border: `1px solid ${C.border}`, marginBottom: "24px" }}>
                    ...
                    {([["Service Fee", `â‚¹${price}`], ["Platform Fee", "â‚¹0"], ["GST (18%)", `â‚¹${gst}`], ["Total", `â‚¹${total}`]] as [string, string][]).map(([k, v], i) => (
